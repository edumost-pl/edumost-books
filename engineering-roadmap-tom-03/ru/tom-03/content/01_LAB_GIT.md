...
📷 **ILL-T3-L1-01 · Машина времени Git**  
**[Для художника]** Подросток за столом; на экране терминал с `git log --oneline`; над столом — **лента commit-ов** как кадры комикса («добавил датчик», «исправил баг», «работает!»); слева — сломанный код, справа — зелёная галочка после отката; подпись: «Каждый commit — точка возврата».
